...
jsPsych.plugins["bandit-task-exp6"] = (function() {
    ...
    plugin.info = {
      ...
      parameters: {
        ...
        estimate: {
          ...
          pretty_name: 'Estimate textbox',
          default: false,
          description: 'Ask participant for value estimate.'
        },
      }
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------
    ...
    plugin.trial = function(display_element, trial) {
      ...
      content.classList.add("banditMag-grid-container");
      ...
      // update instructionText 
       instruction = trial.instructionText;
      ...
      display_element.innerHTML = `
          ...
          <div id="winBlock" class="grid-item winBlock">${instruction}</div>
          <div id="leftOption" class="grid-item options"></div> 
          <div id="rightOption" class="grid-item options"></div>
          <div id="estimatePrompt" class="grid-item estimatePrompt"></div>
  ...
  // audio html
      ...
      var estimatePrompt = display_element.querySelector('#estimatePrompt');
  ...
  // FUNCTIONS -------------------------------------------------------------------------------------------------------
      ...
      function displayChoice() {
        ...
        // stimulus${(stimuli.order[0]+1)} -> option1 or option2

        if (trial.estimate == true){ // add textboxes depending on whether estimate item set as true
          leftOption.innerHTML += `<div id = "leftText"><input type="text"  minlength="1" id="leftTextBox" name="leftTextName"  autocomplete="off" onkeypress="return isNumber(event)"></div>`;
          rightOption.innerHTML += `<div id = "rightText"><input type="text"  minlength="1" id="rightTextBox" name="rightTextName"  autocomplete="off" onkeypress="return isNumber(event)"></div>`;
          estimatePrompt.innerHTML = `<div id = "estimatePrompt"><b>Imagine that this was a WIN trial. </b><br>How many points do you think you would get from each option? <br> Enter your estimate for each option in the textbox beneath it, before making your choice.</div>`;
      ...
      function chooseStimulus1() {
        if (checkTextBoxes()){  // this is the same as saying if checkTextBoxes function returns true
          makeChoice(selection=1)
        }
      } // if the function is true
      function chooseStimulus2() {
        if (checkTextBoxes()){ 
          makeChoice(selection=2)
        }
      }
  
      // Function for checking if there is mandatory responses in the textboxes
      function checkTextBoxes() { // returns boolean
        if (trial.estimate == true){
          leftInput = document.getElementById("leftTextBox").value
          rightInput = document.getElementById("rightTextBox").value
      ...
      function cueHTML(selection){
        ...
        estimatePrompt.style.display = "none";
      ...
      function calculateOutcome(selection, displayedCue) {
        ...
        if (outcome == 100 | outcome == 95 | outcome == 90 | outcome == 80 | outcome == 105 | outcome == 110 |outcome == 120){
      ...
      function nextButton() {
        ...
        next.addEventListener("click", function (){
          /*if (trial.estimate == true){
            if (leftInput == "" | rightInput == ""){
              alert("Please enter your estimates before making your choice.")
            } else{
            endTrial()
            }
          } else{
      ...
      function endTrial() {
        ...
        content.classList.remove("banditMag-grid-container")
